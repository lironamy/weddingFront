...
const Loader = () => {
  ...
  useEffect(() => {
    ...
    const opts = {
      ...
      gravity: 0.1,
      upFlow: -0.1,
      letterContemplatingWaitTime: 100,
      balloonSpawnTime: 20,
    ...
    const calc = {
      totalWidth: opts.charSpacing * Math.max(
    ...
    ctx.font = opts.charSize + 'px Comic Sans MS ';
    ...
    Letter.prototype.step = function () {
      if (this.phase === 'firework') {
        ...
        if (this.circleCreating) {
          ...
          const armonic =
            -Math.cos(proportion * Math.PI) / 2 + 0.5;
    ...
    const anim = () => {
      ...
      ctx.translate(w - hw, hh);
      let done = true;
      for (let l = 0; l < letters.length; ++l) {
        letters[l].step();
        if (letters[l].phase !== 'done') done = false;
      }
      ctx.translate(hw - w, -hh);
      if (!done) {
    ...
    const handleResize = () => {
      ...
      const newHh = h / 2;
      ctx.font = opts.charSize + 'px Comic Sans MS';
      ctx.translate(newHw - hw, newHh - hh);
      hw = newHw;
      hh = newHh;
    };
